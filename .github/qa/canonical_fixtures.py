...
class OAuthHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if route == "/v0/management/routing/strategy":
            self.send_json(200, {"strategy": ROUTING_STRATEGY})
            return
        if route != "/v0/management/auth-files":
    ...
    def do_PUT(self):
        ...
        if self.path.split("?", 1)[0] != "/v0/management/routing/strategy":
            self.send_json(404, {"error": "not found"})
            return
        requested = self.read_json().get("value")
        if requested not in ("round-robin", "fill-first"):
            self.send_json(400, {"error": "invalid routing strategy"})
            return
        ROUTING_STRATEGY = requested
        self.send_json(200, {"status": "ok"})

    def do_POST(self):
        if self.path.split("?", 1)[0] != "/v0/management/auth-files/refresh":
            self.send_json(404, {"error": "not found"})
            return
        self.read_json()
        self.send_json(200, {"status": "ok", "refreshed": 5, "skipped": 0, "files": [], "failed": []})

    def do_PATCH(self):
        route = self.path.split("?", 1)[0]
        self.read_json()
        if route == "/v0/management/auth-files/fields":
            self.send_json(200, {"status": "ok"})
            return
        if route == "/v0/management/auth-files/status":
            self.send_json(200, {"status": "ok", "disabled": False})
            return
        self.send_json(404, {"error": "not found"})
...
def main():
    ...
    oauth = ThreadingHTTPServer(("127.0.0.1", OAUTH_PORT), OAuthHandler)
