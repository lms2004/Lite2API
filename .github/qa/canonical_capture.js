...
(async () => {
  ...
  const routeModelOptions = await page.locator('#routeCreateModelSelect option').evaluateAll(options => options.map(option => option.value));
  if (!routeModelOptions.includes('gpt-5.6-codex-fast')) throw new Error(`route model picker did not expose discovered models: ${routeModelOptions.join(', ')}`);
  if ((await page.locator('#routeCreateModelSelect').inputValue()) !== 'gpt-5.6-codex-fast') throw new Error('route model picker did not select the discovered model');
  await page.locator('#routeCreateModelSelect').selectOption('__lite2api_custom_model__');
  if (await page.locator('#routeCreateCustomModelField').isHidden()) throw new Error('manual model choice did not expose its input');
  await page.locator('#routeCreateModel').fill('fixture-manual-model');
  await page.locator('#routeCreateModelSelect').selectOption('gpt-5.6-codex-fast');
  if (!(await page.locator('#routeCreateCustomModelField').isHidden())) throw new Error('catalog model choice did not close the manual input');
  if ((await page.locator('#routeCreateModel').inputValue()) !== 'gpt-5.6-codex-fast') throw new Error('catalog choice did not update the route model');
  ...
  const targetModelPicker = page.locator('#routeEditor [data-target-index="0"] [data-target-model-choice]');
  if (!(await targetModelPicker.count()) || (await targetModelPicker.inputValue()) !== 'gpt-5.6-codex-fast') throw new Error('saved direct route did not retain a usable model picker');
