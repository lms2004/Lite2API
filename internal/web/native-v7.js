/* Lite2API Native v7 — capability-aware route selection.
   The legacy selects remain the business source of truth; operators interact
   with a searchable grouped model picker and explicit reasoning controls. */
(() => {
  "use strict";

  const BUILD = "Native 7.0 · 2026.08.18";
  const ui = {
    activeModelSelect: null,
    query: "",
    group: "all",
    scheduled: false,
    signature: ""
  };
  const $ = id => document.getElementById(id);
  const all = (selector, root = document) => Array.from(root.querySelectorAll(selector));
  const later = fn => requestAnimationFrame(() => requestAnimationFrame(fn));

  function capabilityAccounts() {
    return Array.isArray(state?.config?.accounts) ? state.config.accounts : [];
  }

  function effortLabel(value) {
    const labels = {
      auto: "自动",
      none: "关闭",
      minimal: "最小",
      low: "低",
      medium: "中",
      high: "高",
      xhigh: "极高",
      max: "最大"
    };
    return labels[value] || value;
  }

  function modelDisplayName(model) {
    const normalized = String(model || "").toLowerCase();
    const names = {
      sol: "GPT-5.6 Sol",
      terra: "GPT-5.6 Terra",
      luna: "GPT-5.6 Luna",
      fast: "Fast",
      "sol-max": "Sol Max"
    };
    return names[normalized] || model;
  }

  function modelGroup(model) {
    const text = String(model || "").toLowerCase();
    if (/^(sol|terra|luna|fast|sol-max)$|gpt|codex|openai/.test(text)) return "openai";
    if (/claude|anthropic/.test(text)) return "claude";
    if (/gemini|google/.test(text)) return "gemini";
    if (/grok|xai|x\.ai/.test(text)) return "grok";
    if (/deepseek/.test(text)) return "deepseek";
    return "other";
  }

  function groupLabel(group) {
    return ({
      openai: "OpenAI / Codex",
      claude: "Claude",
      gemini: "Gemini",
      grok: "Grok",
      deepseek: "DeepSeek",
      other: "其他"
    })[group] || "其他";
  }

  function buildCatalog() {
    const byModel = new Map();
    for (const account of capabilityAccounts()) {
      const capabilities = Array.isArray(account.capabilities) ? account.capabilities : [];
      for (const capability of capabilities) {
        const model = String(capability.model || "").trim();
        if (!model) continue;
        let entry = byModel.get(model);
        if (!entry) {
          entry = {
            model,
            group: modelGroup(model),
            sources: new Set(),
            upstreams: new Set(),
            efforts: new Set(),
            tags: new Set()
          };
          byModel.set(model, entry);
        }
        entry.sources.add(account.name || account.id || "上游");
        if (capability.upstream_model) entry.upstreams.add(capability.upstream_model);
        for (const effort of capability.reasoning_efforts || []) entry.efforts.add(effort);
        const featureText = `${model} ${capability.upstream_model || ""}`.toLowerCase();
        if (/fast|priority/.test(featureText)) entry.tags.add("Fast");
        if (/thinking|reasoner/.test(featureText)) entry.tags.add("Thinking");
        if (/image|vision/.test(featureText)) entry.tags.add("Vision");
      }
    }

    // Keep the current route value selectable even while discovery is stale.
    all("#routeRows .route-intent select:first-of-type").forEach(select => {
      const model = select.value;
      if (model && !byModel.has(model)) {
        byModel.set(model, {
          model,
          group: modelGroup(model),
          sources: new Set(),
          upstreams: new Set(),
          efforts: new Set(),
          tags: new Set(["当前"])
        });
      }
    });

    return Array.from(byModel.values()).sort((a, b) => {
      const groupOrder = ["openai", "claude", "gemini", "grok", "deepseek", "other"];
      const groupDelta = groupOrder.indexOf(a.group) - groupOrder.indexOf(b.group);
      if (groupDelta) return groupDelta;
      return modelDisplayName(a.model).localeCompare(modelDisplayName(b.model), "zh-CN", { numeric: true });
    });
  }

  function catalogSignature() {
    return buildCatalog().map(entry => [
      entry.model,
      Array.from(entry.sources).sort().join(","),
      Array.from(entry.efforts).sort().join(","),
      Array.from(entry.tags).sort().join(",")
    ].join("|")).join("||");
  }

  function ensureModelDialog() {
    let dialog = $("v7ModelDialog");
    if (dialog) return dialog;
    dialog = document.createElement("dialog");
    dialog.id = "v7ModelDialog";
    dialog.className = "v7-model-dialog";
    dialog.setAttribute("aria-labelledby", "v7ModelDialogTitle");
    dialog.innerHTML = `
      <div class="v7-model-dialog-head">
        <div><strong id="v7ModelDialogTitle">选择模型</strong><span>只显示当前上游实际发现或已配置的能力</span></div>
        <button type="button" class="icon-btn" data-v7-close aria-label="关闭">×</button>
      </div>
      <div class="v7-model-search-wrap">
        <input id="v7ModelSearch" type="search" autocomplete="off" placeholder="搜索模型、上游或真实模型 ID" aria-label="搜索模型">
      </div>
      <div id="v7ModelGroups" class="v7-model-groups" role="tablist" aria-label="模型分组"></div>
      <div id="v7ModelResults" class="v7-model-results"></div>
      <div class="v7-model-dialog-foot"><span>模型目录由上游自动同步；路由保存前不会影响当前流量。</span></div>`;
    document.body.append(dialog);
    dialog.querySelector("[data-v7-close]").addEventListener("click", () => dialog.close());
    dialog.addEventListener("click", event => {
      if (event.target === dialog) dialog.close();
    });
    $("v7ModelSearch").addEventListener("input", event => {
      ui.query = event.target.value || "";
      renderModelDialog();
    });
    return dialog;
  }

  function openModelDialog(select) {
    ui.activeModelSelect = select;
    ui.query = "";
    ui.group = "all";
    const dialog = ensureModelDialog();
    $("v7ModelSearch").value = "";
    renderModelDialog();
    if (!dialog.open) dialog.showModal();
    later(() => $("v7ModelSearch")?.focus());
  }

  function renderModelDialog() {
    const catalog = buildCatalog();
    const groups = ["all", ...Array.from(new Set(catalog.map(entry => entry.group)))];
    const groupNode = $("v7ModelGroups");
    const resultNode = $("v7ModelResults");
    if (!groupNode || !resultNode) return;

    groupNode.replaceChildren(...groups.map(group => {
      const button = document.createElement("button");
      button.type = "button";
      button.className = "v7-model-group";
      button.setAttribute("role", "tab");
      button.setAttribute("aria-selected", String(ui.group === group));
      button.textContent = group === "all" ? `全部 ${catalog.length}` : groupLabel(group);
      button.addEventListener("click", () => {
        ui.group = group;
        renderModelDialog();
      });
      return button;
    }));

    const query = ui.query.trim().toLocaleLowerCase("zh-CN");
    const filtered = catalog.filter(entry => {
      if (ui.group !== "all" && entry.group !== ui.group) return false;
      if (!query) return true;
      const haystack = [
        entry.model,
        modelDisplayName(entry.model),
        ...entry.sources,
        ...entry.upstreams,
        ...entry.tags
      ].join(" ").toLocaleLowerCase("zh-CN");
      return haystack.includes(query);
    });

    if (!filtered.length) {
      resultNode.innerHTML = `<div class="v7-model-empty">没有匹配模型</div>`;
      return;
    }

    const fragment = document.createDocumentFragment();
    let previousGroup = "";
    for (const entry of filtered) {
      if (ui.group === "all" && entry.group !== previousGroup) {
        const heading = document.createElement("div");
        heading.className = "v7-model-group-heading";
        heading.textContent = groupLabel(entry.group);
        fragment.append(heading);
        previousGroup = entry.group;
      }
      const button = document.createElement("button");
      button.type = "button";
      button.className = "v7-model-result";
      button.setAttribute("aria-current", String(ui.activeModelSelect?.value === entry.model));
      const sources = Array.from(entry.sources);
      const efforts = Array.from(entry.efforts).filter(Boolean);
      const tags = Array.from(entry.tags);
      button.innerHTML = `
        <span class="v7-model-result-main"><strong></strong><small></small></span>
        <span class="v7-model-result-meta"></span>`;
      button.querySelector("strong").textContent = modelDisplayName(entry.model);
      button.querySelector("small").textContent = sources.length
        ? `${sources.length} 个上游 · ${sources.slice(0, 2).join("、")}${sources.length > 2 ? "…" : ""}`
        : "当前配置";
      const meta = button.querySelector(".v7-model-result-meta");
      if (tags.length) {
        for (const tag of tags.slice(0, 2)) {
          const chip = document.createElement("span");
          chip.className = tag === "Fast" ? "v7-model-tag fast" : "v7-model-tag";
          chip.textContent = tag;
          meta.append(chip);
        }
      }
      if (efforts.length > 1) {
        const chip = document.createElement("span");
        chip.className = "v7-model-tag";
        chip.textContent = `${efforts.length - (efforts.includes("auto") ? 1 : 0)} 档推理`;
        meta.append(chip);
      }
      button.addEventListener("click", () => chooseModel(entry.model));
      fragment.append(button);
    }
    resultNode.replaceChildren(fragment);
  }

  function chooseModel(model) {
    const select = ui.activeModelSelect;
    if (!select) return;
    if (!Array.from(select.options).some(option => option.value === model)) {
      const option = new Option(model, model);
      select.add(option);
    }
    select.value = model;
    select.dispatchEvent(new Event("change", { bubbles: true }));
    $("v7ModelDialog")?.close();
    schedule();
  }

  function enhanceModelSelect(select) {
    if (select.dataset.v7Model === "1") {
      const trigger = select.parentElement?.querySelector(".v7-model-trigger");
      syncModelTrigger(select, trigger);
      return;
    }
    select.dataset.v7Model = "1";
    select.classList.add("v7-native-select");
    const trigger = document.createElement("button");
    trigger.type = "button";
    trigger.className = "v7-model-trigger";
    trigger.setAttribute("aria-haspopup", "dialog");
    trigger.addEventListener("click", () => openModelDialog(select));
    select.after(trigger);
    syncModelTrigger(select, trigger);
  }

  function syncModelTrigger(select, trigger) {
    if (!trigger) return;
    const catalog = buildCatalog();
    const entry = catalog.find(item => item.model === select.value);
    const sources = entry ? entry.sources.size : 0;
    const efforts = entry ? Array.from(entry.efforts).filter(value => value !== "auto") : [];
    trigger.replaceChildren();
    const copy = document.createElement("span");
    copy.className = "v7-model-trigger-copy";
    const strong = document.createElement("strong");
    strong.textContent = modelDisplayName(select.value || "选择模型");
    const small = document.createElement("small");
    small.textContent = `${sources || 0} 个兼容上游${efforts.length ? ` · ${efforts.length} 档推理` : ""}`;
    copy.append(strong, small);
    const arrow = document.createElement("span");
    arrow.className = "v7-model-trigger-arrow";
    arrow.textContent = "⌄";
    trigger.append(copy, arrow);
  }

  function enhanceEffortSelect(select) {
    if (select.dataset.v7Effort === "1") {
      syncEffortControl(select);
      return;
    }
    select.dataset.v7Effort = "1";
    select.classList.add("v7-effort-select");
    const control = document.createElement("div");
    control.className = "v7-effort-control";
    control.setAttribute("role", "radiogroup");
    control.setAttribute("aria-label", "推理强度");
    select.after(control);
    syncEffortControl(select);
  }

  function syncEffortControl(select) {
    const control = select.parentElement?.querySelector(".v7-effort-control");
    if (!control) return;
    const values = Array.from(select.options).map(option => option.value).filter(Boolean);
    control.replaceChildren(...values.map(value => {
      const button = document.createElement("button");
      button.type = "button";
      button.className = "v7-effort-option";
      button.setAttribute("role", "radio");
      button.setAttribute("aria-checked", String(select.value === value));
      button.textContent = effortLabel(value);
      button.addEventListener("click", () => {
        select.value = value;
        select.dispatchEvent(new Event("change", { bubbles: true }));
        schedule();
      });
      return button;
    }));
  }

  function enhanceRouteControls() {
    all("#routeRows .route-intent").forEach(intent => {
      const selects = all("select", intent);
      if (selects[0]) enhanceModelSelect(selects[0]);
      if (selects[1]) enhanceEffortSelect(selects[1]);
    });
  }

  function updateBuildIdentity() {
    document.documentElement.dataset.ui = "native-v7";
    const build = $("uiBuild");
    if (build) build.textContent = "UI build 2026.08.18-v7";
  }

  function sync() {
    ui.scheduled = false;
    updateBuildIdentity();
    enhanceRouteControls();
    const signature = catalogSignature();
    if (signature !== ui.signature) {
      ui.signature = signature;
      if ($("v7ModelDialog")?.open) renderModelDialog();
      all("#routeRows .route-intent select:first-of-type").forEach(select => syncModelTrigger(select, select.parentElement?.querySelector(".v7-model-trigger")));
    }
  }

  function schedule() {
    if (ui.scheduled) return;
    ui.scheduled = true;
    later(sync);
  }

  function wrap(name) {
    const original = window[name];
    if (typeof original !== "function" || original.__nativeV7Wrapped) return;
    const wrapped = function (...args) {
      const result = original.apply(this, args);
      if (result && typeof result.finally === "function") result.finally(schedule);
      else schedule();
      return result;
    };
    Object.defineProperty(wrapped, "__nativeV7Wrapped", { value: true });
    window[name] = wrapped;
  }

  function init() {
    ensureModelDialog();
    ["render", "renderRoutes", "showView", "saveRoutes"].forEach(wrap);
    const rows = $("routeRows");
    if (rows) new MutationObserver(schedule).observe(rows, { childList: true, subtree: true, attributes: true, attributeFilter: ["value", "hidden"] });
    document.addEventListener("change", event => {
      if (event.target.closest?.("#routeRows")) schedule();
    });
    sync();
  }

  window.Lite2APINativeV7 = Object.freeze({
    version: BUILD,
    openModelPicker: openModelDialog,
    sync
  });

  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", init, { once: true });
  else init();
})();
