...
推荐路由在顶层声明逻辑 `model` 与 `reasoning_effort`，`targets[]` 只包含真实渠道。渠道账号通过 `capabilities[]` 把逻辑组合映射到自身的上游模型 ID，因此同一逻辑模型可以由 Antigravity、Claude Code 官方、Web 代理等不同凭据来源实现。调度器先排除不支持该模型或强度的渠道，再按路由策略选择首个健康且有容量的目标：空策略严格保留数组顺序，`priority` 按外层账号 priority 小值优先，其他策略可按负载、轮询或会话哈希选号。失败后只排除当前目标并继续；显式目标集合会完整执行，不受旧的账号数或 `max_failover_attempts` 静默截断。

CLIProxyAPI 渠道内部还有独立的 OAuth 凭据调度层。该层的账号 `priority` 为高值优先：先取支持当前模型且未禁用、未冷却的最高优先级桶，再在桶内使用 `round-robin` 或 `fill-first`；最高桶全部不可用时才落到下一优先级。会话粘性只能保持在当前最高的可用桶内，绑定账号失效后会重选，因此不会阻止故障换号。外层显式路由策略、Lite 连接 `priority` 的低值优先和内层 OAuth `priority` 的高值优先是三个不同的控制面。

认证池默认不限制跨凭据尝试数量（`max-retry-credentials: 0`），但每次选择仍只考虑当前模型真正可用的凭据；托管配置不在一轮结束后等待重试（`request-retry: 0`），让外层目标链立即接管跨渠道 failover。管理 API 更新同级策略时先写盘，失败则回滚内存值，成功后直接替换运行中 selector；不能依赖文件 watcher 比较共享配置指针。替换带会话粘性的 selector 会显式停止旧缓存清理协程，避免反复切换策略造成后台协程泄漏。

只有没有 `capabilities[]` 的旧账号才使用目标级 `model` / `reasoning_effort`。新结构把意图与实现分开：用户调整推理强度后，候选渠道集合会随能力矩阵变化，不需要手工填写或理解渠道专用模型 ID。

以下策略同时适用于旧式 `accounts[]` 路由和显式 `targets[]`；显式目标额外支持空策略表示严格数组顺序：
...
- 能证明请求尚未写入上游的 DNS、连接与 TLS 失败：安全换号。
- 401/402/403：上游已明确拒绝执行，立即短期熔断该账号并换号；402 覆盖订阅或支付额度耗尽。
- 429：上游已明确限流，遵守秒数或 HTTP-date 形式的 `Retry-After`（最长 24 小时），立即熔断并换号。
- 显式目标返回 404（目标模型不存在）：安全尝试下一个目标，不把普通客户端 4xx 扩散到全部渠道。
- 任意上游 POST 收到 408/409/425 或任意 5xx（含 529），以及请求写入后发生超时、重置等传输错误：即使是 Embeddings、Rerank 也可能已被接收或计费，因此一律不跨账号重放；网关返回 `uncertain_submission` 并记录失败。
...
内置协议不增加进程。需要 OAuth、Cookie 或逆向协议的渠道才使用回环地址上的隔离进程。管理端读取适配器目录时才执行本机探针：同一身份 singleflight、最多 4 路并行、整体预算 750 ms（HTTP client 500 ms）、成功缓存 60 秒且失败最多 5 秒，不跟随重定向，也不访问非 loopback 地址。带 Key 的适配器还必须用鉴权请求取得非空且有界的模型目录才判定 ready；普通模型请求完全不执行探针。状态被拆分为安装状态、进程状态、就绪状态和流量状态，避免把“已安装”误判为“已有凭据且可承载请求”。
