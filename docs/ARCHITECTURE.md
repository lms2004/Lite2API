...
- 401/402/403：立即短期熔断该账号并换号；402 覆盖订阅或支付额度耗尽。
- 429：遵守秒数或 HTTP-date 形式的 `Retry-After`（最长 24 小时），立即熔断并换号。
- 408/409/425 和全部 5xx（含提供方常见的 529 overload）：立即尝试下一目标，并计入连续失败；达到阈值后熔断。
