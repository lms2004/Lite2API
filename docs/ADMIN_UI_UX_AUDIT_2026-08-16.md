...
4. 请求质量与 P95 延迟拆成两张图；缺失时间桶留空，不把未知画成 0。
...
处理：拆为“请求质量”和“P95 延迟”两张独立图；空时间桶断线，并提供屏幕阅读器可读的文本摘要。
...
| 成功率 | 严格最近 5 分钟；无样本时为“—” | 判断错误是否影响用户 |
| P95 延迟 | 严格最近 5 分钟；无样本时为“—” | 发现尾延迟 |
| 并发利用率 | 当前活跃请求 / 启用账号有界并发总和；存在无限并发时不计算百分比 | 发现容量逼近上限 |
...
本轮将四段历史样式合并为一个 `<style>`，并建立统一设计令牌：

- 背景：近黑海军蓝。
- 面板：单层深灰蓝，不使用玻璃拟态。
- 强调色：绿色仅表示健康/成功；蓝色表示导航、选择与主操作；黄/红表示风险。
- 圆角：主面板收敛到 10px。
- 阴影：常规面板取消阴影，弹窗保留层级阴影。

OAuth、手动账号、路由、Key、导入导出、Prompt 检测与适配器仍保留在原生 HTML/CSS/JS 技术栈中，没有引入大型框架。
...
- 未知：路由覆盖完整，但最近 5 分钟没有可用于验证的请求。
- 就绪：每条路由至少有目标，并由最近成功请求验证，且窗口内没有失败。
- 降级：存在最近失败或部分目标不可用，但仍有可尝试目标。
- 不可用：任意已配置路由无目标、目标缺失，或没有可尝试目标。
...
- 改变模型或强度后，页面按渠道 `capabilities[]` 移除不兼容项，但不会静默补入全部兼容渠道；保存前展示目标链变化与影响，服务端再次校验。
...
创建时必须选择安全预设：个人开发（30 天、120 RPM、并发 2）、临时测试（24 小时、30 RPM、并发 1）或可信服务（90 天、600 RPM、并发 16）。默认不再是无限权限、永不过期。

明文出现后可复制客户端配置，并用 `GET /v1/models` 做无模型调用成本的连接验证，形成“创建 Key → 复制配置 → 验证连接”的连续流程。
...
- 服务重启后历史清零。
- 低流量时严格保持“最近 5 分钟”窗口；过期记录不会进入健康结论，无样本明确显示未知。
- 无法提供小时/天/周趋势。
- 无法计算严格的时间加权 QPS。
...
- 请求、账号和 Key 表格在窄屏重排为单轴记录，不依赖纯横向滚动。
- 手机端导航固定底部，只保留总览、路由、上游、客户端四个核心入口。
- 健康结论和路由健康在窄屏自动降列。

弹窗通过原生 `dialog` 约束焦点，并在关闭后恢复触发元素；图表已经提供可读文本摘要。仍建议在发布流程中加入 axe-core，并使用 VoiceOver/NVDA 做人工复核。
...
最终实现另保留真实 Chromium 验收证据：总览覆盖 320、768、1024、1440px；Key 流程覆盖 320px；含一条无样本路由的路由页覆盖 320、1024px。截图位于 `docs/design/qa-*.png`，其中无样本结论均为中性“未知”，不使用绿色“正常”状态。
...
- `git diff --check`
- JavaScript 语法检查。
- Chromium 在 320px、768px、1024px、1440px 四档完成响应式截图验证。

### 功能验收

- 管理会话可自动建立。
- 核心四个视图与“诊断与系统”中的低频视图均可切换，刷新后保留当前 hash。
- 运行总览在有/无请求、有/无账号、熔断状态下均不报错。
- 路由修改后出现保存状态；成功保存后恢复已同步。
- 切换逻辑模型或推理强度后只移除不兼容渠道，不静默加入新渠道；保存前展示影响，不支持组合在浏览器与后端均被拒绝。
- 同一模型可配置到多个渠道，顺序调整后保存的数组顺序就是运行时尝试顺序。
- OAuth、手动账号、Key、导入导出、JSON 编辑行为不回归。
- 320px 起导航、指标、洞察面板、记录列表和弹窗可用。

## 10. 后续路线图

### 下一步（高价值、低风险）

- 将目前已合并的样式与内联脚本进一步拆为独立小文件（需同步调整 Go embed/CSP）。
- 增加路由详情侧栏，展示真实失败切换链。
