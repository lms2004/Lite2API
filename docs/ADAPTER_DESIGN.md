...
普通请求热路径只增加一次内存中的字符串能力匹配，不执行探针、不读取磁盘。管理端 `/adapters` 才探测固定 loopback URL：相同适配器和凭据代际以 singleflight 合并，最多 4 路并行，连接超时 250 ms、响应头超时 300 ms、HTTP client 超时 500 ms、排队加执行整体预算 750 ms；成功结果缓存 60 秒，失败最多缓存 5 秒，不跟随重定向。配置探针 Key 的适配器必须以该 Key 读取非空、限长的 `/models` 响应才可进入 ready。单机不引入 Redis、数据库、队列、注册中心或高频健康轮询。
